...
function DigitalDateTime(id, options, streamBundle, instrumentPanel) {
  ...
  class ReactComponent extends React.Component {
    ...
    render() {
      ...
      return (
        <svg key={id} onClick={this.changeDateFormat} height="100%" width="100%" viewBox="0 0 20 35">
          <SvgTouchApp24px x="-22" y="0" width="0.35em" height="0.4em"/>
          ...
          <text x="10" y="15" textAnchor="middle" fontSize={textDate.fontSizeDate} dominantBaseline="middle">{textDate.date}</text>
          <text x="10" y="29" textAnchor="middle" fontSize="16" dominantBaseline="middle">{textDate.time}</text>
        </svg>
      )
    }
  }
