...
function Attitude(id, options, streamBundle, instrumentPanel) {
  ...
  class ReactComponent extends React.Component {
    ...
    render() {
      return (
        <svg key={id} height="100%" width="100%" viewBox="0 0 20 35" >
          <text x="10" y="6" textAnchor="middle" fontSize="6" dominantBaseline="middle">{this.props.options.label}</text>
          <text x="-25" y="14" textAnchor="start" fontSize="10" dominantBaseline="middle">yaw:</text>
          <text x="-25" y="23" textAnchor="start" fontSize="10" dominantBaseline="middle">pitch:</text>
          <text x="-25" y="31" textAnchor="start" fontSize="10" dominantBaseline="middle">roll:</text>
          <text x="45" y="14" textAnchor="end" fontSize="10" dominantBaseline="middle">{this.state.yaw}</text>
          <text x="45" y="23" textAnchor="end" fontSize="10" dominantBaseline="middle">{this.state.pitch}</text>
          <text x="45" y="31" textAnchor="end" fontSize="10" dominantBaseline="middle">{this.state.roll}</text>
        </svg>
      )
    }
  }
